...
async def create_invoice(amount):
    ...
    data = {'asset': 'USDT', 'amount': str(amount), 'description': 'DX9WARE', 'allow_comments': False}
...
async def process_callback(callback_query: types.CallbackQuery):
    ...
    if callback_query.data == 'shop':
        markup = types.InlineKeyboardMarkup(row_width=2).add(
            types.InlineKeyboardButton("Android", callback_data="shop_apk"),
        ...
        if res['ok']:
            markup = types.InlineKeyboardMarkup(row_width=1).add(
                ...
                types.InlineKeyboardButton("✅ Проверить", callback_data=f"check_{res['result']['invoice_id']}")
            )
            await bot.send_message(uid, f"Счет на {PRICES[plan]}$ создан:", reply_markup=markup)
        ...
        if await check_invoice(inv_id):
            ...
            if new_key:
                ...
                await bot.send_message(uid, f"✅ Оплата прошла!\n\nТвой ключ: `{new_key}`\nАктивируй его в меню.", parse_mode="Markdown")
            else:
                await bot.send_message(uid, "❌ Оплата ок, но ключи кончились. Пиши: " + SUPPORT_URL)
        else:
            await bot.answer_callback_query(callback_query.id, "Не оплачено!", show_alert=True)
    elif callback_query.data == 'shop_apk':
        markup = types.InlineKeyboardMarkup(row_width=1).add(
            types.InlineKeyboardButton("7 дней (4$)", callback_data="buy_apk_7"),
            types.InlineKeyboardButton("Месяц (8$)", callback_data="buy_apk_30"),
            types.InlineKeyboardButton("⬅️ Назад", callback_data="shop")
        )
        await bot.edit_message_text("Цены Android:", uid, callback_query.message.message_id, reply_markup=markup)
    elif callback_query.data == 'shop_ios':
        markup = types.InlineKeyboardMarkup(row_width=1).add(
            types.InlineKeyboardButton("7 дней (6$)", callback_data="buy_ios_7"),
            types.InlineKeyboardButton("Месяц (12$)", callback_data="buy_ios_30"),
            types.InlineKeyboardButton("⬅️ Назад", callback_data="shop")
        )
        await bot.edit_message_text("Цены iOS:", uid, callback_query.message.message_id, reply_markup=markup)
        ...
        await bot.send_message(uid, "Отправь ключ:")
...
@dp.message_handler()
async def check_user_key(message: types.Message):
