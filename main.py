...
async def cmd_rem_key(message: types.Message):
    if not is_admin(message.from_user.id): return
    key = message.get_args()
    conn = sqlite3.connect("database.db")
    conn.execute("UPDATE users SET active_until=NULL, current_key=NULL, access_blocked=1 WHERE current_key=?", (key,))
    conn.commit(); conn.close()
    await message.answer(f"🚫 Ключ `{key}` отвязан. Доступ юзерам закрыт до /backKey.")
...
async def cmd_back_key(message: types.Message):
    ...
    conn = sqlite3.connect("database.db")
    conn.execute("UPDATE users SET access_blocked=0 WHERE id=?", (uid,))
    conn.commit(); conn.close()
    await message.answer(f"✅ Доступ для ID `{uid}` восстановлен.")
...
async def cmd_aprofile(message: types.Message):
    ...
    await message.answer(f"🛠 **Admin View {target.mention}:**\nID: `{u[0]}`\nKey: `{u[4]}`\nExp: `{u[2]}`", parse_mode="Markdown")
...
async def admin_main(message: types.Message):
    ...
    await message.answer("🔮 **ADMIN PANEL | DX9 WARE**", reply_markup=markup, parse_mode="Markdown")
...
async def adm_k3(message: types.Message, state: FSMContext):
    ...
    await message.answer("Кол-во активаций (по умолчанию 1):")
...
async def shop_menu(message: types.Message):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(types.InlineKeyboardButton("🤖 Android (APK)", callback_data="buy_and"),
               types.InlineKeyboardButton("🍎 iOS", callback_data="buy_ios"))
    await message.answer("🛒 **Выберите вашу платформу:**", reply_markup=markup, parse_mode="Markdown")

@dp.callback_query_handler(lambda c: c.data.startswith('buy_'))
async def shop_prices(call: types.CallbackQuery):
    p = "Android" if "and" in call.data else "iOS"
    markup = types.InlineKeyboardMarkup(row_width=2)
    if p == "Android":
        markup.add(types.InlineKeyboardButton("7 Days (4$)", callback_data="pay_4"),
                   types.InlineKeyboardButton("Month (8$)", callback_data="pay_8"))
    else:
        markup.add(types.InlineKeyboardButton("7 Days (6$)", callback_data="pay_6"),
                   types.InlineKeyboardButton("Month (12$)", callback_data="pay_12"))
    await call.message.edit_text(f"🔹 **Платформа:** {p}\nВыбери срок подписки:", reply_markup=markup, parse_mode="Markdown")
...
async def create_invoice(call: types.CallbackQuery):
    ...
    markup.add(types.InlineKeyboardButton("💎 Crypto Pay", url=inv.pay_url))
    markup.add(types.InlineKeyboardButton("⭐️ Оплата Звёздами", url=f"https://t.me/{PAYMENT_USER.replace('@','')}"))
    await call.message.answer(f"✅ Чек на **{amount}$** создан!\nДля оплаты звёздами или подтверждения пиши {PAYMENT_USER}", reply_markup=markup)
...
async def support(message: types.Message): await message.answer(f"🆘 Поддержка: {SUPPORT_USER}")

@dp.message_handler(lambda m: m.text == "📢 Канал")
async def channel(message: types.Message): await message.answer(f"📢 Канал: {CHANNEL_URL}")

@dp.message_handler(lambda m: m.text == "💬 Отзывы")
async def reviews(message: types.Message): await message.answer(f"💬 Отзывы: {REVIEWS_URL}")
...
async def global_handler(message: types.Message):
    ...
    if u[5] == 1: return await message.answer("❌ Доступ заблокирован.")
    ...
    if p:
        if p[1] == "type_disc":
            ...
            await message.answer(f"🎁 Скидка {p[2]}% активирована!")
        elif p[1] == "type_time":
            end = (datetime.now() + timedelta(days=int(p[2]))).strftime("%Y-%m-%d")
            cur.execute("UPDATE users SET active_until=? WHERE id=?", (end, uid))
            await message.answer(f"✅ Доступ до `{end}` по промо!")
    ...
    if k:
        if k[3] < k[2]:
            ...
            conn.commit(); await message.answer(f"🚀 Успешно! Подписка до `{end}`")
        else:
            await message.answer("❌ Ключ уже использован кем-то другим.")
    conn.close()
