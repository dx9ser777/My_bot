...
if not API_TOKEN or not CRYPTO_PAY_TOKEN:
    logging.error("ОШИБКА: Токены не найдены в Секретах Amvera!")
    ...
    if callback_query.data == 'shop':
        ...
        if await check_invoice(inv_id):
            ...
            if new_key:
                ...
                await bot.send_message(uid, f"✅ Оплата прошла!\nКлюч: `{new_key}`", parse_mode="Markdown")
            else:
                await bot.send_message(uid, "❌ Ключи закончились. Пиши " + PAYMENT_ADMIN)
        else:
        ...
        if uid in active_users and os.path.exists(FILE_NAME):
            with open(FILE_NAME, 'rb') as f:
                await bot.send_document(uid, f, caption="Файлы DX9WARE готов!")
